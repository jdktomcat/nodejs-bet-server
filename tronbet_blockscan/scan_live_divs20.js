...
const EVENT_SNAPSHOT_STARTED = sha3.keccak256('SnapshotStarted(uint32,uint64,uint64,uint32)');
const EVENT_SNAPSHOT_COMPLETED = sha3.keccak256('SnapshotCompleted(uint32,uint64,uint64,uint32)');
const EVENT_LIVE_DIVIDEND_CREATE = sha3.keccak256('LiveDividendCreate(uint256,uint64,uint256,uint256[],uint32,uint32)');
const EVENT_LIVE_DIVIDEND_STEP = sha3.keccak256('LiveDividendStep(uint256,uint256,address,uint64)');
const EVENT_LIVE_DIVIDEND_COMPLETE = sha3.keccak256('LiveDividendComplete(uint256,uint256,uint256[],uint32,uint32)');
const EVENT_LIVE_DIVIDEND_TRANSFERTRC20 = sha3.keccak256('TransferTRC20(address,address,uint256)');

// LiveStakingDividend
const LIVE_STAKING_DIVIDEND = conf.tronConfig.live_staking_dividend;
// USDT
const USDT_ADDRESS = conf.tronConfig.usdt_address;

const scanBlocknumFileName = 'scan_live_divs20.blocknum';

console.log('EVENT_SNAPSHOT_STARTED', EVENT_SNAPSHOT_STARTED);
console.log('EVENT_SNAPSHOT_COMPLETED', EVENT_SNAPSHOT_COMPLETED);
console.log('EVENT_LIVE_DIVIDEND_CREATE', EVENT_LIVE_DIVIDEND_CREATE);
console.log('EVENT_LIVE_DIVIDEND_STEP', EVENT_LIVE_DIVIDEND_STEP);
console.log('EVENT_LIVE_DIVIDEND_COMPLETE', EVENT_LIVE_DIVIDEND_COMPLETE);
console.log('EVENT_LIVE_DIVIDEND_TRANSFERTRC20', EVENT_LIVE_DIVIDEND_TRANSFERTRC20);
...
async function alysisTxs(tx) {
  ...
  for (let _contract of contract) {
    ...
    if (contract_address === LIVE_STAKING_DIVIDEND) {
      //玩家下单之后事件通知
        ...
        for (let _log of logs) {
          ...
          if (eventCode === EVENT_SNAPSHOT_STARTED) {
            let log = {
              ...
              devStakingAmount: hexStringToBigNumber(hexData.substr(128, 64)).toNumber(),
              stakerCnt: hexStringToBigNumber(hexData.substr(192, 64)).toNumber()
            };
            redis.publish('tron_live_trc20_dividends', JSON.stringify(log));
            console.log(log);
            ...
            redis.publish('tron_live_trc20_dividends', JSON.stringify(log));
            console.log(log);
          } else if (eventCode === EVENT_LIVE_DIVIDEND_CREATE) {
            console.log(_log);
            let log = {
            ...
            redis.publish('tron_live_trc20_dividends', JSON.stringify(log));
            console.log(log);
          } else if (eventCode === EVENT_LIVE_DIVIDEND_STEP) {
            let log = {
            ...
            for (let l of logs) {
              if (`41${l.address}` == LIVE_STAKING_DIVIDEND) {
                let topics = l.topics;
                let data = l.data;

                let event = topics[0];
                if (event === EVENT_LIVE_DIVIDEND_TRANSFERTRC20) {
            ...
            redis.publish('tron_live_trc20_dividends', JSON.stringify(log));
          } else if (eventCode === EVENT_LIVE_DIVIDEND_COMPLETE) {
            let log = {
...
async function saveRoundInfo(info, trc20Addr, amount) {
  ...
  let sql = `INSERT INTO tron_live.live_trc20_div_info
...
async function saveRoundDetail(info, trc20Addr, amount) {
  ...
  try {
    let sql = `INSERT INTO tron_live.live_trc20_div_detail
...
async function saveCompleteInfo(info) {
  try {
    let sql = `UPDATE tron_live.live_trc20_div_info SET state = 2, end = ? WHERE round = ?;`;
...
async function main() {
  ...
  while (true) {
    ...
    if (result) {
      ...
      console.log('-----finish scan_live_divs20 block num ', blockNum, ' in ', new Date().getTime() - stime, 'ms');
      blockNum += 1;
    } else {
      console.log('-----------scan_live_divs20 restart blockNum : ', blockNum);
      await sleep(3000);
    }
  }
}
