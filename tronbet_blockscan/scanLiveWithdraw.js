...
const scanBlocknumFileName = 'scanLiveWithdraw.blocknum';

console.log('EVENT_LIVE_USER_BUY_BOUNS', EVENT_LIVE_USER_BUY_BOUNS);
console.log('EVENT_LIVE_USER_WITHDRAW1', EVENT_LIVE_USER_WITHDRAW);
    ...
    if (contract_address === PAY_ORDER_ADDR || contract_address === WITHDRAW_ADDR) {
      ...
      if (logs && _.isArray(logs) && !_.isEmpty(logs)) {
        for (let _log of logs) {
          ...
          if (eventCode === EVENT_LIVE_USER_WITHDRAW) {
            console.log(_log);
            let log = {
...
async function saveLiveUserRechargeLog(info) {
  let sql = 'insert into tron_live.live_charge_log(addr, amount, txId, ts) values (?, ?, ?, ?)';
  let sql2 = 'insert into tron_live.live_user(addr, trx) values (?,?) ON DUPLICATE KEY update trx = trx + ?;';
  let conn = null;
  try {
    ...
    await execTrans(sql, [info._fromAddr, info._amount, info._txId, info._ts], conn);
    await execTrans(sql2, [info._fromAddr, info._amount, info._amount], conn);
    conn.commit();
  } catch (e) {
    console.log(e);
...
async function saveLiveUserWithdrawLog(info) {
  try {
    let sql = 'update tron_live.live_withdraw_log set status = 1, txId = ? where addr = ? and orderId = ?';
    let res = await query(sql, [info._tx_id, info._fromAddr, info._orderId]);
  } catch (error) {
    console.log(error);
    return false;
  }
