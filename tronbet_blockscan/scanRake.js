...
const EVENT_Divide = sha3.keccak256('RakeDivide(uint256,uint256)');
const EVENT_CODE_DivideCreate = sha3.keccak256('RakeDivideCreate(uint64,uint64,uint64,uint64,uint64)');
const EVENT_CODE_DivideComplete = sha3.keccak256('RakeDivideComplete(uint64,uint64,uint64,uint64,uint64)');
const EVENT_CODE_DivideStep = sha3.keccak256('RakeDivideStep(uint256,address,uint64,uint64,uint64,uint64,uint256,uint256)');
const EVENT_RakeAirDrop = sha3.keccak256('RakeAirDrop(address,uint256,uint128,uint128,uint256)');

// TronBetRakeDivider
const DIVIDEND_ADDR = conf.tronConfig.tron_bet_rake_divider;
// TronBetTexas
const PAY_ADDR = conf.tronConfig.tron_bet_texas;
// TronBetRakePool
const RAKE_POOL_ADDR = conf.tronConfig.tron_bet_rake_pool;

const scanBlocknumFileName = 'scanRake.blocknum';
...
console.log('EVENT_RakeAirDrop', EVENT_RakeAirDrop);
...
async function alysisTxs(tx) {
  ...
  for (let _contract of contract) {
    ...
    if (contract_address === DIVIDEND_ADDR || contract_address === PAY_ADDR || contract_address === RAKE_POOL_ADDR) {
      //玩家下单之后事件通知
      ...
      if (logs && _.isArray(logs) && !_.isEmpty(logs)) {
        for (let _log of logs) {
          ...
          if (eventCode === EVENT_Divide) {
            ...
            redis.publish('tron_rake_dividends', JSON.stringify(log));
            console.log(log);
          ...
          } else if (eventCode === EVENT_RakeAirDrop) {
            let log = {
...
async function saveAirDropLog(info) {
  try {
    let sql = 'insert into tronbet_poker_log.rake_airdrop_log(addr, startTs, endTs, betAmount,adAmount, txId, status, confirmedStatus) values(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE adAmount = ?, txId = ?, status = 1,confirmedStatus = 1';
    await db.exec(sql, [info.player, info.fromSec, info.toSec, 0, info.tokenAmount, info.txID, 1, 1, info.tokenAmount, info.txID]);
  } catch (error) {
    console.log(error);
    return false;
  }
...
async function saveRoundInfo(info) {
  let sqlDividendsInsert = 'insert into tronbet_poker_log.rake_div_info (round,total_token,total_trx,send_ts,div_state, rank_trx) values (?,?,?,?,?,?) ON DUPLICATE KEY UPDATE total_token = ?,total_trx=?,send_ts=?,div_state=?';
  try {
    let hotRank = Math.floor((info.trxAmountTotal * 0.03) / 0.97);
    await db.exec(sqlDividendsInsert, [info.round, info.tokenAmountTotal, info.trxAmountTotal, info.tmCreate, 1, hotRank, info.tokenAmountTotal, info.trxAmountTotal, info.tmCreate, 1]);
  } catch (e) {
    console.log(e);
    return false;
  }
...
async function saveRoundDetail(info) {
  try {
    let sqlDividendsDetail = 'insert into tronbet_poker_log.rake_div_detail (round,addr,token,trx,total_token, total_trx,state,tx_id) values (?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE tx_id = ?';
    let res = await db.exec(sqlDividendsDetail, [info.round, info.owner, info.tokenAmount, info.trxAmount, info.tokenAmountTotal, info.trxAmountTotal, 3, info.txID, info.txID]);
  } catch (error) {
    console.log(error);
    return false;
  }
...
async function saveCompleteInfo(info) {
  try {
    let sqlDividendsUpdate = 'update tronbet_poker_log.rake_div_info set div_state = 2 where round = ?;';
    await db.exec(sqlDividendsUpdate, [info.round]);
  } catch (error) {
    console.log(error);
    return false;
  }
...
async function main() {
  ...
  while (true) {
    ...
    if (result) {
      ...
      console.log('-----finish block num ', blockNum, ' in ', new Date().getTime() - stime, 'ms');
      blockNum += 1;
    } else {
      console.log('-----------restart blockNum : ', blockNum);
      await sleep(3000);
    }
  }
}
