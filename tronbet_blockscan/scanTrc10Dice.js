...
const EVENT_DiceCreate10 = sha3.keccak256('DiceCreate10(uint256,address,uint256,uint16,uint8,uint256)');
const EVENT_DiceResult10 = sha3.keccak256('DiceResult10(uint256,address,uint256,uint256,uint256,uint256,uint256,uint256)');

const TRC10_DICE_ADDR = conf.tronConfig.trc10_bet_addr;
const TRC10_DICE_BET_RESULT_ADDR = conf.tronConfig.trc10_oracle_addr;

const scanBlocknumFileName = 'scanTrc10Dice.blocknum';
...
async function alysisTxs(tx) {
  ...
  for (let _contract of contract) {
    ...
    if (contract_address === TRC10_DICE_ADDR || contract_address == TRC10_DICE_BET_RESULT_ADDR) {
      //玩家下单之后事件通知
      ...
      if (logs && _.isArray(logs) && !_.isEmpty(logs)) {
        for (let _log of logs) {
          ...
          } else if (eventCode === EVENT_DiceResult10) {
            console.log(_log);
            let log = {
              _orderId: hexStringToBigNumber(hexTopics[1].substr(0, 64)).toNumber(),
              _fromAddr: hexStringToTronAddress(hexTopics[2].substr(24, 40)),
...
async function saveAnteDiceOrder(info) {
  ...
  let sql = 'insert into tron_bet_wzc.trc10_dice_order_v2(addr, order_id, token, amount, direction, number, ts, order_tx, sign) values (?, ?, ?, ?, ?, ?, ?, ?,?) ON DUPLICATE KEY update ts = ?, order_tx = ?, sign = ?';
  let conn = null;
  try {
...
async function saveAnteDiceResult(info) {
  let sql = 'update tron_bet_wzc.trc10_dice_order_v2 set roll = ?, win = ?, result_tx = ? where addr = ? and order_id = ?';
  let conn = null;
  try {
    conn = await getConnection();
    if (conn == null) {
...
async function main() {
  ...
  while (true) {
    ...
    if (result) {
      ...
      console.log('-----finish scan trc10 block num ', blockNum, ' in ', new Date().getTime() - stime, 'ms');
      blockNum += 1;
    } else {
      console.log('-----------restart trc10 blockNum : ', blockNum);
      await sleep(3000);
    }
  }
}
