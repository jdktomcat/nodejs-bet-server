...
async function getPreRoundInfo(_ver, conn) {
    let sql = "select * from win_ver_v1 where ver = ? limit 1;"
    let params = [_ver];
...
async function getLastestRoundInfo(conn) {
    let sql = "select * from win_ver_v1 order by ver desc limit 1;"
    let params = [];
    let fun = conn == null ? exec : execTrans;
    let ret = await fun(sql, params, conn);
    // console.log(ret[0]);
...
async function updateLastestRoundInfo(round, btt, tx, state, conn) {
    let sql = "update win_ver_v1 set btt_total = ?,btt_tx = ?,btt_state = ? where ver = ?;"
...
async function getBttRoundInfo(_ver, conn) {
    let sql = "select * from win_dividends_v1 where ver = ? and state = 3 and btt_state <> 3 limit 1000;";
    let params = [_ver];
    let fun = conn == null ? exec : execTrans;
    let ret = await fun(sql, params, conn);
    return ret;
...
async function addBttTxInfo(round, addr, amount_sun, total_sun, tx, state, conn) {
    let sql = "update win_dividends_v1 set btt = ?, btt_total = ?, btt_tx = ?, btt_state = ? where ver = ? and addr = ?;";
...
async function updateBttTxInfo(round, addr, state, conn) {
    let sql = "update win_dividends_v1 set btt_state = ? where ver = ? and addr = ?;";
...
async function updateAllBttTxState(round, state, conn) {
    let sql = "update win_dividends_v1 set btt_state = ? where ver = ? and btt_state = 0;";
