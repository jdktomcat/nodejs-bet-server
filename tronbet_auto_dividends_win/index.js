...
async function onComplete(event_complete) {
    ...
    allHadSendTs = 0;
    ignore_ts = 0;
    loggerDefault.info("event complete!!!", isBusy, event_complete);
    trc10.sendTrc10(event_complete.round); //以win分红版本来进行btt分红
}
...
function subscribe() {
    ...
    redisObserver.on('message', (channel, message) => {
        ...
        switch (_type) {
            case "DividePlusCreate": //win分红事件名称修改 DividePlusCreate
                appEvent.emit("create", 0, message.maxHolderIndex);
                break;
            case "DividePlusComplete": //win分红事件名称修改 DividePlusComplete
                //{ _type: 'dividends_complete',round: 230,trxAmountTotal: 882882589731,anteAmountTotal: 73097242902584,maxHolderIndex: 16188,tmCompleted: 1562386026 }
                appEvent.emit("complete", message);
                break;
            case "DividePlusStep": //win分红事件名称修改 DividePlusStep
...
async function app() {
    ...
    trc10.sendTrc10(); //尝试补发TRC10（仅补发，不新建）
}
