...
export default class extends Base {
  ...
  async basedataAction() {
    ...
    let totalDivdiedSql = "select sum(total_trx / 1000000) totalAnte from (select * from ante_ver union all select * from ante_ver_v1) a"
    ...
    if (game == 'All' || game == 'Dice') {
        ...
        let totlalAmountsql = "select all_bet_sun - 379417986000000 amount_sun from dice_block order by block_num desc limit 1;"
        ...
        result = {
            ...
            totalDivdied : totalDivdied[0].totalAnte,
        }
    }

    if (game == 'All' || game == 'Moon') {
        let totlalAmountsql = "select sum(amount) totlalAmount, count(distinct addr) totalUser from tron_bet_admin.moon_user_order"
    ...
    if (game == 'All' || game == 'Ring') {
        let totlalAmountsql = "select count(1) totalUser, sum(wheel_total) totlalAmount from dice_players where wheel_total > 0"
  ...
  async rankAction() {
    ...
    console.log(game, '------------------------')
    if (game != 'All' && game != 'Dice' && game != 'Moon') {
        return this.success([])
    }
