...
async function preDivide(round) {
    ...
    tronSrv.commitTransaction(pay_addr, "Divide(uint256,uint256)", 20000000, 0, [{ type: "uint256", value: balance }, { type: "uint256", value: round + 20 }], defaultPK, (err, res) => {
        if (err) {
    ...
    let result = await tronSrv.tronQuery(pay_addr, "isSysTxConfirmed(uint256)", [{ type: "uint256", value: round + 20 }])
    if (result.result.result === true) {
...
function subscribe() {
    ...
    redisObserver.subscribe('tron_rake_dividends', (err, res) => {
        if (err) {
            loggerError.error(err);
            loggerDefault.info("订阅频道 [tron_rake_dividends] 失败");
            process.exit(0);
        }
        loggerDefault.info("订阅频道 [tron_rake_dividends] 成功");
    });
