...
const hosts = conf.tronConfig.masterFullNode
...
const scanBlocknumFileName = 'scanFull.blocknum';
...
async function getBlockData(blockNum) {
    let {data} =  await xhr({
        url : hosts + '/wallet/getblockbynum',
        method : 'post',
        data : {
            num: blockNum
        }
...
async function alysisBlockData(blockData, blockNumber) {
    ...
    if (transactions && !_.isEmpty(transactions) && _.isArray(transactions) && transactions.length > 0) {
        ...
        logger.info("-----------------------------------------------------------------------------------------------------------------");
        logger.info("FULLNODE区块:" + blockNumber + "; 出块时间:" + moment(ts).format('YYYY-MM-DD HH:mm:ss') + "; transactionnum:" + txCount + '; blockId:' + blockData.blockID);
        await Promise.all(transactions.map(async (txInfo) => {
            ...
            if (!tmp) {
                logger.info('-------------------full----blockNumber----------failed---', blockNumber)
                result = false
            }
        }))
    }
...
async function gettransactioninfobyid(txID){
    let {data} = await xhr({
        url : hosts + '/wallet/gettransactioninfobyid',
        method : 'post',
        data : {
            value : txID
        }
        ...
        if (contract_address === JACKPOT_ADDR) { //
            ...
            for (let _log of logs) {
                ...
                if (eventCode === ReachJackPort_EVENT) {
                    ...
                    //     _winner: hexStringToTronAddress(hexData.substr(24, 40)),
                    //     _cardShow: hexStringToBigNumber(hexData.substr(64, 64)).toNumber(),
                    //     _trxAmount: hexStringToBigNumber(hexData.substr(128, 64)).toNumber(),
                    //     _pollAmount: hexStringToBigNumber(hexData.substr(192, 64)).toNumber(),
                    //     _ts: timestamp,
                    //     _txId: txID,
                    // }
                    // console.log(log)
                    ...
                    //     return false
                    // }
                    console.log('full jack reached, nothing to do')
...
async function SaveReachJackpotAction(info) {
    let sql = "insert into poker_jackpot_log(jackpotId, winner, cardType, trx, pool, reachTs, reachTx) values(?,?,?,?,?,?,?) ON DUPLICATE KEY update reachTs = ?;"
    let res = await db.exec(sql, [info._jackportId, info._winner, info._cardShow, info._trxAmount, info._pollAmount, info._ts, info._txId, info._ts])
    await updateRedisData(info._winner)
    return true
}

async function SaveGetJackpotAction(info) {
    let sql = "update poker_jackpot_log set getTs = ?, getTx = ?, status = 1 where jackpotId = ?"
...
async function main() {
    ...
    while (true) {
        ...
        if (result) {
            ...
            logger.info('-----finish full block num ', blockNum, ' in ', new Date().getTime() - stime, 'ms')
            blockNum += 1
        } else {
            logger.info('-----------restart full blockNum : ', blockNum)
            await sleep(3100)
        }
