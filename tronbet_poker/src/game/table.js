...
const DEFAULT_BUYIN_DURATION = 180000; //买入等待时间(毫秒)
...
class tableInfo extends events {
    constructor(options = {}) {
        ...
        //info
        this.gameType = options.gameType || GAME_TYPE.CASH;
        ...
        //盲注信息
        this.blindList = {
            ANTE: this.config.ante, //前注
            SB: this.config.smallBlind, //小盲
            BB: this.config.bigBlind, //大盲
            STRADDLE: this.config.straddle, //抓盲
            MISSBB: this.config.bigBlind, //补盲
        }
    ...
    load(_config, _blindList) { //从mongodb加载配置项
        if (_config == null || _blindList == null) return;
        this.config = _config;
        this.blindList = _blindList;
        let _now = _.now();
            ...
            this.allSeats.findAll(null, (o) => {
                ...
                if (player != null) {
                    if (player.playerState === PLAYER_STATE.LEFT) {
                        // if (player.todo.toSeatBack === true) { //点击回桌后，不再计算当前是否站起超时（如离桌倒计时0.1秒）
                        //     // if (player.getBalance() > 0 && player.sitOutTs > 0) player.sitOutTs = 0;
                        // } else if (player.sitOutTs > 0 && (now - player.sitOutTs > DEFAULT_LEFT_MAX)) { //站起超时
                        //     gameMessage.info(this.tableId, this.no, "===> 站起超时>>踢出" + player.position + "(" + player.uname + ")" + " toSeatBack:" + player.todo.toSeatBack + " toSitOut:" + player.todo.toSitOut);
                        //     this.playerExit(player.uid);
                        // } else {
                        //     // gameMessage.info(this.tableId,this.no,"站起 => 即将超时" + player.position +","+ player.uname +","+ (now - player.sitOutTs) +","+ DEFAULT_LEFT_MAX);
                        // }

                        if (player.sitOutTs > 0 && (now - player.sitOutTs > DEFAULT_LEFT_MAX)) { //站起超时
                            if (player.todo.toSeatBack === true && (this.tableState === TABLE_STATE.PLAYING || this.tableState === TABLE_STATE.CLEARING)) { //牌局还在继续游戏且离桌玩家返回中
                                gameMessage.info(this.tableId, this.no, "===> 站起超时>>不过正在回座中");
                            } else {
                                gameMessage.info(this.tableId, this.no, "===> 站起超时>>踢出" + player.position + "(" + player.uname + ")" + " toSeatBack:" + player.todo.toSeatBack + " toSitOut:" + player.todo.toSitOut);
                                this.playerExit(player.uid);
                            }
    ...
    gameover() {
        gameMessage.info(this.tableId, this.no, "\r\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< GAMEOVER (SHOW RANK) >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> \r\n");
        this.getRankInfo(null, true); //广播排行榜数据(比赛结束)
    }
    checkCountPlayer() {
        ...
        if (total_players < this.config.minPlayers) {
            gameMessage.info(this.tableId, this.no, "===> 玩家 < " + this.config.minPlayers + "人 ==> !!!!!!!!! 游戏暂停 !!!!!!!!!");
            this.tableState = TABLE_STATE.PAUSE;
            this.onPause();
    ...
    start(restart) {
        ...
        // console.log("AAA", lastSBSeatNo, lastBBSeatNo);
        // console.log("BBB", _count_players_playing);
        ...
        if (total_players === 2) {
            ...
            let needPay4MissBB = this.tableState === TABLE_STATE.WAITING ? false : true; //WAITING => PLAYING 第一次开桌，当前有效玩家全部直接上桌
        ...
        if (nextBTNSeat != null) this.gameInfo.btnSeatNo = nextBTNSeat.data.player.seatNo;
        if (nextSBSeat != null) this.gameInfo.sbSeatNo = nextSBSeat.data.player.seatNo;
        if (nextBBSeat != null) this.gameInfo.bbSeatNo = nextBBSeat.data.player.seatNo;

        // nextBTNSeat.data.player.init_balance = 100500000; //open --> allin  8 --> 100
        // nextBBSeat.data.player.init_balance = 1380000001;
        //     nextBTNSeat.data.player.init_balance = 3750450000;
        //牌桌开始游戏
    ...
    stop() {
        let t = setTimeout(() => {
            ...
            if (this.tableState === TABLE_STATE.STOP) {
                ...
                this.getRankInfo(null, false); //广播排行榜数据(比赛进行中)
                let q = setTimeout(() => {
                    clearTimeout(q);
                    this.start();
                }, 1000);
    ...
    playerAutoBet(uid, op, arrLogs) {
        ...
        let player = this.getPlayerByUID(uid);
        let op_value = this.blindList[op];
    ...
    //直接离桌
    playerExit(uid) {
        if (this.gameType === GAME_TYPE.CUP) return;
        ...
        if (this.tableState === TABLE_STATE.WAITING || this.tableState === TABLE_STATE.STOP || this.tableState === TABLE_STATE.PAUSE) {
            gameMessage.info(this.tableId, this.no, "===> 牌桌已停/暂停 => 立刻退桌 uid = " + uid + " name = " + player.uname);
            this.kickOut(player);
        } else {
            if (player.playerState !== PLAYER_STATE.PLAYING) {
                gameMessage.info(this.tableId, this.no, "===> 牌桌正常 => 玩家非游戏中 => 立刻退桌 uid = " + uid + " name = " + player.uname);
                // player.foldAndExitASAP = true;
                this.kickOut(player);
            } else if (player.playerState === PLAYER_STATE.PLAYING) { //正在玩
                // if (player.isActionComplete(this.roundInfo.roundIndex, this.roundInfo.roundMaxBet)) { //非活牌
                //     gameMessage.info(this.tableId,this.no,"游戏中，行动已结束=>直接退桌" + uid);
                //     this.kickOut(player);
                // } else {
                gameMessage.info(this.tableId, this.no, "===> 牌桌正常 => 玩家正在游戏中 => 行动未结束 => 尽快退桌 uid = " + uid + " name = " + player.uname);
                player.foldAndExitASAP = true; //TODO:如果this.tableState === TABLE_STATE.CLEARING 时可能会失效 => 无法尽快离桌，下一局走超时流程
                let curActor = this.getPlayerByUID(this.roundInfo.curActorUid);
                if (curActor != null && curActor.uid === player.uid) {
                    this.playerBet(player.uid, 0, true);
                }
                // }
            } else {
                gameMessage.info(this.tableId, this.no, "===> 牌桌正常 => 无法退桌" + player.position + "(" + player.uname + ") balance = " + player.getBalance() + "; playerState = " + player.playerState);
            }
        }
    }
    waitingForBB(uid) {
    ...
    getRankInfo(socket_id, gameover) {
        ...
        if (socket_id != null) {
            ...
            this.sendRankInfo(socket_id, ret);
        } else {
            ret.gameover = gameover || false;
    ...
    async orderConfirmed(order) {
        ...
        if (player != null && player.seatNo >= 0 && commonEnum.PLAYER_CAN_PAY(this.tableState, player.playerState)) {
            ...
            if (oldBalance < maxBuyIn && LobbyInstance.lobbyState != LOBBY_STATE.STOP) {
                ...
                let backAmount = 0;
                if (existOrder.trxAmount > canAdd) {
                    backAmount = existOrder.trxAmount - canAdd;
                }

                player.init_balance += (existOrder.trxAmount - backAmount);
                ...
                if (backAmount < existOrder.trxAmount) {
                    this.playerConfirm(existOrder.addr, existOrder.trxAmount - backAmount);
                    logService.addConfirmLog(existOrder, oldBalance, player.init_balance, backAmount, player.seatNo, this.no);
                ...
                loggerDefault.warn('[确认充值订单1], txID:%s, trx:%s, balance:%s->%s, backAmount:%s', txID, existOrder.trxAmount, oldBalance, player.init_balance, backAmount);
            } else {
