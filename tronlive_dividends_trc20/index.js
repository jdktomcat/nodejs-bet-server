...
const ignoreTestRound = config.app.ignoreTestRound;
...
// 从链上获取质押信息
function getLiveStakerStateFromBlock() {
  return new Promise((resolve, reject) => {
    tronSrv.queryTransaction(staker_addr, 'isPaused()', [], async (err, result) => {
      if (err) reject(err);
      if (result == null || result.result == null) {
        loggerDefault.warn('getLiveStakerStateFromBlock() -> isPaused() 查询结果为空!');
        reject('Data is empty');
      }
      if (result.result.result === true) {
        let data = result.constant_result[0];
...
async function askForDivide(round) {
  loggerDefault.info('askForDivide() -> round: ', round);
  let balance = await dbService.getProfitAmount();
  // let balance = { amount: 200 };
  round = round + ignoreTestRound;

  let usdtBalance = balance.amount || 0;
  usdtBalance = Math.floor(usdtBalance * 1e6 * 0.5);

  loggerDefault.info('askForDivide() -> round, usdtBalance: ', round, usdtBalance);

  if (test_trx > 0) {
    usdtBalance = test_trx * 1e6;
    console.warn('askForDivide() -> 测试分红 TRX: ' + test_trx);
  }

  if (usdtBalance < min_pool_trx) {
    isBusy = false;
    return console.warn('askForDivide() !!! 奖池太低,暂不分红');
  }
  let trc20Arr = [];
  let usdtParam = config.app.TRC20TokenType.usdt.addr;
  let usdtVal = new BigNumber(usdtBalance).toString(16);
  if (usdtVal.length > 22) {
    usdtVal = 'ffffffffffffffffffffff';
    console.info('askForDivide() -> usdtVal 超过最大值');
  } else {
    usdtParam += Array(22 - usdtVal.length)
      .fill(0)
      .join('');
  }
  usdtParam += usdtVal;
  trc20Arr.push('0x' + usdtParam);

  isBusy = true;
  tronSrv.commitTransaction(
    pay_addr,
    'askForDivide(uint256,uint256[],uint256)',
    20000000,
    0,
    [{ type: 'uint256', value: 0 }, { type: 'uint256[]', value: trc20Arr }, { type: 'uint256', value: round }],
    defaultPK,
    async (err, res) => {
      if (err) {
        loggerError.error('askForDivide() -> Divide fail: ', err);
        isBusy = false;
      }
      if (res.result === true) {
        loggerDefault.info('askForDivide() -> Divide success: ', res);

        let result = await tronSrv.tronQuery(pay_addr, 'confirmedTxMap(uint256)', [{ type: 'uint256', value: round }]);
        if (result.result.result === true) {
          let isSuccess = tronSrv.hexStringToBigNumber(result.constant_result[0].substr(0, 64)).toNumber();
          loggerDefault.info('askForDivide() -> operatorTicket: ', isSuccess);
          if (isSuccess) {
            await sleep(200000);
            await createStakeSnapshot();
          } else {
            isBusy = false;
          }
        }
        ...
        isBusy = false;
        loggerDefault.error('askForDivide() -> Divide fail: ', res);
      }
    }
  );
}
...
function scan() {
  let timer = setTimeout(async () => {
    ...
    if (now < roundInfo.start) {
      ...
      // 先判断质押状态, 再转账, 后分红
      try {
        let { isPaused } = await getLiveStakerStateFromBlock();
        if (!isPaused) {
          isBusy = true;
          await askForDivide(roundInfo.round);
        }
      } catch (error) {
        loggerDefault.info('scan() -> can not get liveStakerState: ', error);
      }
    }
    return appEvent.emit('scan');
  }, 3000);
...
function subscribe() {
  ...
  redisObserver.subscribe('tron_live_trc20_dividends', (err, res) => {
    if (err) {
      loggerError.error(err);
      loggerDefault.info('subscribe() -> 订阅频道 [tron_live_trc20_dividends] 失败');
      process.exit(0);
    }
    loggerDefault.info('subscribe() -> 订阅频道 [tron_live_trc20_dividends] 成功');
  });
