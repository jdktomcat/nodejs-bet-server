...
const dice20Orcale = config.dice.dice20Orcale;
const dice20Addr = config.dice.dice20Addr;
...
async function resolve(_address, _order_id, _direction, _number, _sunAmount, _random, _nodeName, _privateKey, _callback) {
    ...
    tronExec(dice20Orcale, "importSeedFromThirdSugar(address,uint256,uint256,uint256)", fee_resolve_max, 0, [
...
function doBetCommon(_tokenAddr, _sunVal, _number, _direction, _privateKey, _callback) {
    let tokenConfig = config.dice.trc20_support[_tokenAddr]
    if (tokenConfig == null || tokenConfig.enable !== true) {
        _callback("not support this trc20 token:" + tokenAddr, null);
        return;
    }
    if (!_.isNumber(_sunVal) || !_.isNumber(_number) || !_.isNumber(_direction) || (_direction === 0 && _number > 95) || (_direction === 1 && _number < 4)) return;
    tronExec(dice20Addr, "GoodLuck(uint256,uint256)", fee_do_bet_max, _sunVal, [{ type: 'uint256', value: _number }, { type: 'uint256', value: _direction }], null, _privateKey, (err, res) => {
        if (!_callback) return;
        if (err) return _callback(err, null);
...
function getOrder(_address, _privateKey, _callback) {
    ...
    tronQuery(dice20Addr, "getOrder(address)", [{ type: "address", value: _address }], null, _privateKey, (err, hexStr) => {
        if (!_callback) return;
        if (err) return _callback(err, null);
        ...
        let data = {
            ...
            tokenAddr: hexStringToTronAddress(hexStr.substr(408, 40)),
        }

        let tokenConfig = config.dice.trc20_support[data.tokenAddr];
        if (tokenConfig && tokenConfig.enable === true) {
            data.trxAmount = data.sunAmount / tokenConfig.decimal;
            _callback(null, data);
        } else {
            _callback(`not support this trc20 token ! token: ${data.tokenAddr}`, null);
        }
...
function getOrderByIdArray(_addressArray, nodeName, _callback) {
    ...
    tronQuery(dice20Addr, "getOrderByIdArray(address[])", [{ type: "address[]", value: _ownerArray }], nodeName, null, (err, hexStr) => {
        if (!_callback) return;
        if (err) return _callback(err, null);
        if (hexStr == null) return _callback("hexStr is empty", null);
        ...
        // let _tokenAddrArray_type = hexStringToBigNumber(hexStr.substr(384, 64)).toNumber();
        ...
        let ret7 = getAddressArr(hexStr, _rollArrayEndIndex);
        let _tokenAddrArray = ret7[0];
        // let _rollArrayEndIndex = ret6[1];
        ...
        for (let idx = 0; idx < _bettorArray.length; idx++) {
            let dice_result = {
                ...
                roll: _rollArray[idx],
                tokenAddr: _tokenAddrArray[idx],
                // trxAmount: _trxAmountArray[idx] / 1000000
            }
            // console.log("dice_result",dice_result);
            let tokenConfig = config.dice.trc20_support[dice_result.tokenAddr];
            if (tokenConfig && tokenConfig.enable === true) {
...
function checkOrder(dice_result, nodeName) {
    ...
    redis.hget("player:logs:trc20:" + addr, order_id, (err, res) => {
...
function resolveAgain(addr, to_do_order_id, direction, number, sunAmount, nodeName = null, pk = defaultPk) {
    redis.hget("player:logs:trc20:" + addr, to_do_order_id, (err, log) => {
        if (err) return;
        if (log != null) {  //已存在待解决的订单
            ...
            if (_.isEmpty(tx)) { //都有tx才对
                ...
                resolve(addr, to_do_order_id, log.direction, log.number, log.sunAmount, log.random, nodeName, pk, (err, res) => {
                    ...
                    log.ts = _now;
                    redis.hset("player:logs:trc20:" + addr, to_do_order_id, JSON.stringify(log));
                });
            } else {
...
function handleOrder(data) {
    ...
    getOrder(addr, null, (err, info) => {
        ...
        if (to_do_order_id === info.orderid) {
            redis.hget("player:logs:trc20:" + addr, to_do_order_id, (err, oldLog) => {
                if (err) return;
                ...
                info.fromDB = false;
                redis.hset("player:logs:trc20:" + addr, to_do_order_id, JSON.stringify(info), (err) => {
                    if (err) return;
                    resolve(addr, to_do_order_id, info.direction, info.number, info.sunAmount, null, null, null, (err, txRes) => {
                        ...
                        redis.hset("player:logs:trc20:" + addr, to_do_order_id, JSON.stringify(info)); //log update random & tx
                    });
                });
            });
        }
...
function chechRollback(addr, to_do_order_id, log, nodeName, pk) {
    getOrder(addr, null, (err, info) => {
        ...
        if (info.orderid < to_do_order_id) {
            ...
            redis.hget("player:logs:trc20:" + addr, roll_back_order_id, (err, pre_log) => {
                ...
                resolve(addr, roll_back_order_id, info.direction, info.number, info.sunAmount, pre_log.random, nodeName, pk, (err, res) => { //使用当前参数，从redis获取对应订单的原种子
                    ...
                    info.ts = _.now();
                    redis.hset("player:logs:trc20:" + addr, to_do_order_id, JSON.stringify(info));
                });
            })
        } else {
...
async function onDiceComplete(dice_result) {
    loggerDefault.info("onDiceCompleteTrc20", dice_result);
    ...
    let completeDiceLog = createBetLogInfo(orderId, bettor, name || "", direction, number, roll,
        trxAmount, payout / 1e6, multiplier, ts, isWin, isHighRoller, isRareWins, "trc20", tokenConfig.name);

    //fill log detail data
    let log = await redis.hget("player:logs:trc20:" + bettor, orderId);
    if (log != null) {
        log = JSON.parse(log);
    ...
    await redis.hset("player:logs:trc20:" + bettor, orderId, JSON.stringify(completeDiceLog));
    let min_order_id = orderId - MY_LOGS_CAP;
    redis.hkeys("player:logs:trc20:" + bettor, (err, res) => {
        if (err == null && res != null) {
            for (let key of res) {
                if (key <= min_order_id) {
                    redis.hdel("player:logs:trc20:" + bettor, key);
                }
            }
        }
    })
